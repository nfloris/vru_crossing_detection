...
class YOLOv5Detector(): 
    ...
    def __init__(self, model_name, confidence = CONFIDENCE_THRESHOLD):
        ...
        self.confidence_threshold = confidence # Minimum theshold for the detection bounding box to be displayed
        self.tracked_classes = self.classes
        self.tracked_classes = list(self.tracked_classes.values())
        self.bbox_color = (217, 217, 217)
    
    def set_bb_color(self, color):
    ...
    def extract_detections(self, results, frame, height, width):
        ...
        for object_index in range(num_objects):
            ...
            if row[4] >= self.confidence_threshold:
                ...
                if self.class_to_label(labels[object_index]) in self.tracked_classes :
                    ...
                    feature = self.class_to_label(labels[object_index])
        ...
        confidences = [detection[-2] for detection in detections]

        return detections , class_count, class_ids
    
    def plot_boxes(self , x1 , y1 , x2 , y2 , frame):  
        cv2.rectangle(frame, (x1, y1), (x2, y2), self.bbox_color, 1)
